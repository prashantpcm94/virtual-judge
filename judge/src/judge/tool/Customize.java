...
public class Customize {
	...
	static public void exe(){
		...
		Cproblem cproblem;
		List<Cproblem> clist = new ArrayList<Cproblem>();
		List<Object[]> list = baseService.query("select cp, p.title from Cproblem cp, Problem p where cp.problemId = p.id ");
		for (Object[] o : list) {
			cproblem = (Cproblem) o[0];
			cproblem.setTitle((String) o[1]);
			clist.add(cproblem);
		}
