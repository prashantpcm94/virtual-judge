...
//		dataList = statService
//				.list(
//						"select vlog, user.username from Vlog vlog, User user where user.id = vlog.loginer order by vlog.createTime desc",
//						0, 25);
//		return SUCCESS;
//	}
//
...
//		dataList = statService
//				.query("select vlog.ip, vlog.userAgent from Vlog vlog order by vlog.createTime desc");
//		Map<String, Integer> map = new HashMap<String, Integer>();
//		Set<String> set = new HashSet<String>();
//
//		for (int i = 0; i < dataList.size(); i++) {
//			if (unique > 0
//					&& set.contains((String) ((Object[]) dataList.get(i))[0]))
//				continue;
...
//		ofcChart = new Chart("Browser Distribution"
//				+ (unique > 0 ? "(By IP)" : "(By Session)")); // 整个图的标题
...
//				"0x339966", "0xCCCC33" });// 饼图每块的颜色
//		c2.setTooltip("#val#  /  #total# -- #percent#\n #label#\n"); // 鼠标移动上去后提示内容
//		ofcChart = new Chart("OS Distribution"
//				+ (unique > 0 ? "(By IP)" : "(By Session)")); // 整个图的标题
//		ofcChart.addElements(c2); // 把饼图加入到图表
//		return SUCCESS;
//	}
...
//		} else {
//			dataList = statService
//					.query("select avg(vlog.duration) from Vlog vlog group by vlog.ip");
//		}
//		long maxx = 1, maxy = 1;
//		for (int i = 0; i < dataList.size(); i++) {
//			long tmp = unique > 0 ? ((Double) dataList.get(i)).longValue()
//					: (Long) dataList.get(i);
//			if (maxx < tmp) {
//				maxx = tmp;
//			}
...
//		int cnt[] = new int[60];
//		for (int i = 0; i < dataList.size(); i++) {
//			long l = unique > 0 ? ((Double) dataList.get(i)).longValue()
//					: (Long) dataList.get(i);
...
//			long s = maxx * i / 50000;
//			bar.setTooltip("stay time: " + (s > 60 ? (s / 60 + "min") : "") + s
//					% 60 + "sec\n" + cnt[i]
//					+ (unique > 0 ? " IPs" : " sessions")); // 鼠标移动上去后的提示
...
//		int maxy = 0;
//		dataList = statService
//				.query("select count(*), day(vlog.createTime), month(vlog.createTime), year(vlog.createTime) from Vlog vlog group by day(vlog.createTime), month(vlog.createTime), year(vlog.createTime)"
//						+ " where vlog.createTime > " + f.format(begin));
//		for (int i = 0; i < dataList.size(); i++) {
//			Object[] o = (Object[]) dataList.get(i);
//			int dif = (int) (((new Date((Integer) o[3] - 1900,
//					(Integer) o[2] - 1, (Integer) o[1])).getTime() - begin
//					.getTime()) / 86400000);
...
//
//		dataList = statService
//				.query("select vlog.id, day(vlog.createTime), month(vlog.createTime), year(vlog.createTime) from Vlog vlog group by vlog.ip, day(vlog.createTime), month(vlog.createTime), year(vlog.createTime)"
//						+ " where vlog.createTime > " + f.format(begin));
//		for (int i = 0; i < dataList.size(); i++) {
//			Object[] o = (Object[]) dataList.get(i);
//			int dif = (int) (((new Date((Integer) o[3] - 1900,
